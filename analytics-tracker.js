...
class KingsaunaAnalytics {
    ...
    async ensureVisitor() {
        ...
        try {
            ...
            // First, fetch current visitor to get current total_sessions
            const { data: currentVisitor } = await this.supabase
                .from('visitors')
                .select('total_sessions')
                .eq('visitor_id', this.visitorId)
                .single();
            
            const newTotalSessions = (currentVisitor?.total_sessions || 0) + 1;
        ...
        try {
            ...
            if (error) {
                ...
                await this.supabase.rpc('increment_page_views', { 
                    ...
                    // Fetch current visitor to get current total_page_views
                    const { data: currentVisitor } = await this.supabase
                        .from('visitors')
                    ...
                    const newTotalPageViews = (currentVisitor?.total_page_views || 0) + 1;
                    
                    await this.supabase
                        .from('visitors')
                        .update({ total_page_views: newTotalPageViews })
                        .eq('visitor_id', this.visitorId);
                });
            }
        } catch (err) {
